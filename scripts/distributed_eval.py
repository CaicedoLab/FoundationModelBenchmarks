...
def eval_checkpoint(q: Queue, model_check, model_arch):
    ...
    if model_arch == 'dinov2':
        ...
        result = subprocess.run(
                    f"extract --root-dir /scr/data/CHAMMI/dataset/ --feat-dir {os.path.join(FEATURES_ROOT, model)} --model channelvit --model-size small --model-path {checkpoint} --gpu {gpu} --batch-size 24 --checkpoint {checkpoint} ",
                    shell=True,
                    # capture_output=True,
                    check=True,
                    text=True
